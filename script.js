...
/* ==== SANDI LOGIN ==== */
const CORRECT_PASSWORD = "Hizz";

const defaultTasks = [
  {
    id: 1,
    title: "Membuat Project Web",
    subject: "MP RPL",
    date: addDays(1),
    priority: "high",
    description: "Membuat project website task management.",
    done: false,
  },
  {
    id: 2,
    ...
    date: addDays(2),
    priority: "medium",
    description: "Mengerjakan latihan soal matematika.",
    done: false,
  },
  {
    id: 3,
    title: "Laporan Praktik",
    subject: "PKKRPL",
    date: addDays(3),
    priority: "high",
    description: "Menyelesaikan laporan praktik.",
    done: false,
  },
  {
    id: 4,
    ...
    date: addDays(5),
    priority: "low",
    description: "Membuat tugas Bahasa Inggris.",
    done: true,
  },
];

/* ==== Penyimpanan aman (tidak crash walau localStorage diblokir) ==== */
const storage = {
  get(key) {
    try {
...
let tasks;
try {
  const saved = JSON.parse(storage.get("taskku_tasks"));
  tasks = Array.isArray(saved) ? saved : defaultTasks;
} catch (err) {
  tasks = defaultTasks;
}
...
let studentName = storage.get("taskku_name") || "";
...
function save() {
  storage.set("taskku_tasks", JSON.stringify(tasks));
}
...
function dateText(s) {
  return parseDate(s).toLocaleDateString("id-ID", {
    day: "numeric",
    month: "short",
    year: "numeric",
  });
...
// Prefill nama kalau pernah login sebelumnya
const savedName = storage.get("taskku_name");
...
function updateTitle() {
  const titles = {
    dashboard: `${greeting()}, ${studentName || "Siswa"} 👋`,
    tasks: "Kelola Tugas",
    calendar: "Kalender & Jadwal",
    progress: "Progress Tugas",
    notifications: "Notifikasi",
  };
  document.getElementById("pageTitle").textContent =
    titles[currentPage] || "TaskKu";
}
function applyUserInfo() {
  document.getElementById("profileName").textContent = studentName || "Siswa";
  document.getElementById("avatarText").textContent = initialsOf(
    studentName || "Siswa",
  );
  document.getElementById("welcomeName").textContent = studentName || "Siswa";
  updateTitle();
}
function enterApp() {
  applyUserInfo();
  loginScreen.classList.add("leaving");
  setTimeout(() => loginScreen.classList.add("gone"), 500);
  setTimeout(() => appRoot.classList.add("show"), 250);
}
function showError(msg) {
  document.getElementById("loginError").textContent = msg;
  loginBox.classList.remove("shake");
  void loginBox.offsetWidth; // reset animasi
...
document.getElementById("loginForm").addEventListener("submit", (e) => {
  ...
  if (pass !== CORRECT_PASSWORD) {
    ...
    passInput.value = "";
    showError("Sandi salah! Coba lagi.");
    passInput.focus();
    return;
  }

  studentName = name;
  storage.set("taskku_name", name);
  enterApp();
});

// Tombol mata untuk lihat/sembunyikan sandi
document.getElementById("togglePass").addEventListener("click", () => {
  const input = document.getElementById("studentPass");
  input.type = input.type === "password" ? "text" : "password";
});

// Bersihkan error saat mulai mengetik lagi
["studentName", "studentPass"].forEach((id) => {
...
document.getElementById("logoutBtn").addEventListener("click", () => {
  if (confirm("Keluar? Kamu akan kembali ke halaman login.")) {
    location.reload();
  }
});
...
function renderStats() {
  ...
  document.querySelector(".donut").style.background =
    `conic-gradient(#3b82f6 ${pct * 3.6}deg,#dce6f6 ${pct * 3.6}deg)`;
  document.querySelector(".big-donut").style.background =
    `conic-gradient(#3b82f6 ${pct * 3.6}deg,#dce6f6 ${pct * 3.6}deg)`;
  document.getElementById("progressBar").style.width = pct + "%";
  document.getElementById("progressText").textContent = total
    ? `${done} dari ${total} tugas sudah selesai.`
    : "Belum ada tugas.";
  document.getElementById("notificationDot").style.display = tasks.some(
    (t) => !t.done && daysUntil(t.date) <= 2,
  )
    ? "block"
    : "none";
}
...
function renderSchedule() {
  ...
  document.getElementById("scheduleGrid").innerHTML = names
    .map(
      (day) => `
...
function calendarCells(date, targetId, big = false) {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    if (big) {
      ...
      html += `<span class="${muted ? "muted" : today ? "today" : ""} ${has ? "has-task" : ""}">${number}</span>`;
    }
  }
...
function renderCalendars() {
  const month = currentDate.toLocaleDateString("id-ID", {
    month: "long",
    year: "numeric",
  });
  ...
  const bigMonth = bigCurrentDate.toLocaleDateString("id-ID", {
    month: "long",
    year: "numeric",
  });
...
function renderDeadlines() {
  const list = tasks
    ...
    .slice(0, 4);
  document.getElementById("deadlineList").innerHTML = list.length
    ? list.map(taskRow).join("")
    : `<div class="empty">🎉 Tidak ada tugas yang mendekati deadline.</div>`;
}
  ...
  document.getElementById("allTasks").innerHTML = filtered.length
    ...
    <div class="task-card">
      ...
      <div class="actions">
        ...
        .join("")
    : `<div class="empty">Tidak ada tugas yang sesuai.</div>`;
}

function renderProgress() {
  const sorted = [...tasks].sort((a, b) => Number(a.done) - Number(b.done));
  document.getElementById("progressTaskList").innerHTML = sorted.length
    ? sorted
        .map(
          (t) => `
    <div class="task-card">
...
function renderNotifications() {
  ...
  list.forEach((t) => {
    ...
    if (d < 0)
      html += `<div class="notification danger"><h3>🚨 Deadline terlewat: ${escapeHtml(t.title)}</h3><p>${escapeHtml(t.subject)} • Deadline ${dateText(t.date)}.</p></div>`;
    else if (d <= 2)
      html += `<div class="notification warning"><h3>⏰ Deadline semakin dekat: ${escapeHtml(t.title)}</h3><p>${escapeHtml(t.subject)} • ${d === 0 ? "Deadline hari ini" : d === 1 ? "Deadline besok" : `Deadline ${d} hari lagi`}.</p></div>`;
  });
  if (!html)
    html = `<div class="notification"><h3>🔔 Semua aman!</h3><p>Tidak ada tugas yang membutuhkan perhatian khusus saat ini.</p></div>`;
...
document.getElementById("taskForm").addEventListener("submit", (e) => {
  ...
  if (existing) tasks = tasks.map((t) => (t.id === existing.id ? data : t));
...
["searchTask", "filterPriority", "filterStatus"].forEach((id) =>
...
document.getElementById("bigPrev").onclick = () => {
  bigCurrentDate.setMonth(bigCurrentDate.getMonth() - 1);
  renderCalendars();
};
